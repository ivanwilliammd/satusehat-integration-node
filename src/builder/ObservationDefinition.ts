...
export class ObservationDefinition {
  private data: Record<string, any> = { resourceType: 'ObservationDefinition' };
  ...
  setCategory(system: string, code: string, display: string = ''): this {
    this.data['category'] = { coding: [{ system, code, display }] };
    return this;
  }

  setMethod(system: string, code: string, display: string = ''): this {
    this.data['method'] = { coding: [{ system, code, display }] };
    return this;
  }
}
